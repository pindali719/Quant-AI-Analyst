...
def project_revenue(latest_revenue: float, growth_rates: list[float]) -> list[float]:
    ...
    for rate in growth_rates:
        latest_revenue=latest_revenue*(1 + growth_rates[rate])
...
def calculate_fair_value_per_share(equity_value, shares_outstanding) -> float:

    fair_value_per_share = equity_value / shares_outstanding

    return fair_value_per_share

def run_dcf(financial, market_data, assumptions) -> dict:
    ...
    latest_revenue=income_statement[0]["TotalRevenue"]
    latest_free_cash_flow=cash_flow[0]["FreeCashFlow"]
    cash=balance_sheet.loc["CashAndCashEquivalents"]
    debt=balance_sheet.loc["TotalDebt"]


    fcf_margin = calculate_fcf_margin(cash_flow=cash_flow, income_statement=income_statement)
